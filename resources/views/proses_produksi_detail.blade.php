...
@section('content')
  <div class="row">
      <div class="col-md-12">
        <div class="x_panel">
          <div class="x_title">
            <a href="{{ url('admin/tambah_proses_produksi_detail') }}" class="btn btn-secondary"><i class="fa fa-plus"></i> Tambah</a>
          </div>
          <div class="x_content">
              ...
              <tbody>
                @foreach($data as $d)
                  <tr>
                    ...
                    <td>{{ $d->shift }}</td>
                    <td>{{ $d->no_mesin }}</td>
                    <td>{{ $d->jam_ambil_sample }}</td>
                    <td>{{ $d->status }}</td>
                    <td>
                      <a href="{{ url('admin/edit_proses_produksi_detail/'.$d->id) }}" class="btn btn-primary btn-sm"><i class="fa fa-edit"></i></a>
...
<script type="text/javascript">
  function del(id) {
    Swal.fire({
      ...
      if (result.value) {
        window.location.href = "{{ url('admin/hapus_proses_produksi_detail') }}"+'/'+id; 
      }
    })
  }
</script>
